...
const NotificationCommand = async () => {
  ...
  const query =
    "select FeedingDevices.title, NotificationAlerts.feeder_id, NotificationAlerts.message_topic, NotificationAlerts.message_text, NotificationAlerts.createdAt " +
    "from NotificationAlerts " +
...
const SystemCheck = async () => {
  ...
  if (systemData != null) {
    ...
    if (hours > 24) {
      ...
      if (hours <= 1) hours_text = hours + " hour";
      else hours_text = hours + " hours";
    }

    if (hours >= 2) {
      messageText =
        "MQTT Server looks like offline / not responding since " + hours_text;
      postSocketNotification([
        {
          title: "All Feeders",
          message_topic: messageTopic,
          message_text: messageText,
          createdAt: moment(now).format("dddd, MMMM D, YYYY, H:mm:ss"),
          feeder_id: "All Feeders",
        },
      ]);
    } else {
      NotificationCommand();
