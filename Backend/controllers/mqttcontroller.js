...
subscribeTopics();
var { io, splitStr, postSocketMessage, postSocketNotification } = require("../socket");
...
client.on('message', async function (topic, message) {
    ...
    let cameraKey = false;
    let myCamAry = [
        "image", "json", "birdalert"
    ]
    if (topic_length >= 4) {
        if (myCamAry.includes(splited_topic[topic_length - 1]) || splited_topic[topic_length - 1] == "capture1" || splited_topic[topic_length - 1] == "capture2") {
            let cameraTitle;
            ...
            if (cameraMacData !== null) {
                ...
                if (myCamAry.includes(splited_topic[topic_length - 1]) && splited_topic[topic_length - 1] != "image") {
            ...
            //     getLastElem = splited_topic[topic_length - 2] + "" + splited_topic[topic_length - 1];
        } else if (topic_length >= 4 || (topic_length == 3 && splited_topic[1] == "ctrl")) {
            getLastElem = splited_topic[topic_length - 2] + "" + splited_topic[topic_length - 1];
        }
    }
    ...
    let newAr = {};
    let newArCapture = {};
    if (cameraKey || getLastElem == "Processed1birdalert" || getLastElem == "Processed2birdalert" || getLastElem == "HB" || getLastElem == "capture1" || getLastElem == "capture2" || getLastElem == "capture3" || getLastElem == "capture4" || getLastElem == "Processed1" || getLastElem == "Processed2" || getLastElem == "Processed1fgmask" || getLastElem == "Processed2fgmask" || getLastElem == "IMAGE" || getLastElem == "Processed1SAM" || getLastElem == "Processed2SAM" || getLastElem == "ctrlHB") {
        if (cameraKey || getLastElem == "Processed1" || getLastElem == "Processed2" || getLastElem == "capture1" || getLastElem == "capture2" || getLastElem == "capture3" || cameraKey || getLastElem == "capture4" || getLastElem == "IMAGE" || getLastElem == "Processed1SAM" || getLastElem == "Processed2SAM") {
            let base64ImageString = Buffer.from(message, 'binary').toString('base64');
            var dir = './public';
            let random = Math.floor(Math.random() * (999999999999 - 111111111111) + 111111111111);
            if (!fs.existsSync(dir)) {
                fs.mkdirSync(dir);
            }

            // if (getLastElem == "Processed1" || getLastElem == "Processed2" || getLastElem == "capture1" || getLastElem == "capture2") {
            if (cameraKey || getLastElem == "Processed1" || getLastElem == "Processed2" || getLastElem == "capture3" || getLastElem == "capture4") {
                let filename = getLastElem + ".png";
                ImageProcessing(message, filename)
                .catch((err) => console.log(`img proc error : ${err}`));
    ...
    if (cameraKey || getLastElem == "capture1" || getLastElem == "capture2" || getLastElem == "Processed1" || getLastElem == "Processed2" || getLastElem == "Processed1fgmask" || getLastElem == "Processed2fgmask" || getLastElem == "Processed1SAM" || getLastElem == "Processed2SAM") {
        let post = { client_message: JSON.stringify(newAr), client_topic: getLastElem, feeder_id: feeder_id };
        ...
        if (getLastElem == "ctrlHB") {
            let post = { message: JSON.stringify(newAr), topic: getLastElem, feeder_id: feeder_id };
            models.ServerMessages.create(post).catch((error) => {
                console.log("error in creation commands" + error)
            });
        } else if (getLastElem == "Processed1birdalert" || getLastElem == "Processed2birdalert" || getLastElem == "Processed1json" || getLastElem == "Processed2json") {
            if (newAr) {
                //isInteger
                if (getLastElem == "Processed1birdalert" || getLastElem == "Processed2birdalert") {
                    ...
                    if (newAr.key != "nobird") {
                        ...
                        let post = { client_message: bf_count, client_topic: getLastElem, feeder_id: feeder_id };
                    ...
                    if (myNewAr.countBirds > 0) {
                        let post = { client_message: JSON.stringify(myNewAr), client_topic: getLastElem, feeder_id: feeder_id };
                        models.BirdsData.create(post).catch((error) => {
                            console.log("error in creation commands" + error)
                        });
                    }
            ...
            if (!toSkip) {
                ...
                if (getLastElem == "sensorstatus" || getLastElem == "sensorworking" || getLastElem == "feedingdone") {
                    let post = { client_message: JSON.stringify(newAr), feeder_id: feeder_id };
                    if (getLastElem == "sensorstatus") {
...
const collectNotifications = async (topic, message, feeder_id) => {
    let post = { client_message: message, client_topic: topic, feeder_id: feeder_id };
    ...
    if (topic == "sensorstatus") {
        ...
        //
        //Battery Charging
        if (message["10"] < 0) { // charging
            messageTopic = "batterycharging"
            messageText = "battery is charging";
            payload = { message_topic: messageTopic, message_text: messageText, feeder_id: feeder_id };
            await models.NotificationAlerts.create(payload);
        } else if (message["10"] > 0) { // charging stopped
            messageTopic = "batterycharging"
        ...
        //
        //Battery Motor
        if (message["13"] > 0) { // motor
            messageTopic = "motor"
            messageText = "motor current stopped";
            payload = { message_topic: messageTopic, message_text: messageText, feeder_id: feeder_id };
            await models.NotificationAlerts.create(payload);
        } else if (message["13"] < 0) { // motor
            messageTopic = "motor"
    ...
    if (topic == "feedingdone") {
        ...
        var dateString = moment.unix(message["65"]).format("dddd, MMMM D, YYYY, H:mm:ss");
