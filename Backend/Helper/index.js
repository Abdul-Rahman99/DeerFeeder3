...
const handleQuantity = (time, qty) => {
    ...
    console.log(time)
    for (let i = 1; i < div; i++) {
        let newHM = moment("2024-02-24 " + time).add(2, "minutes").format("H:mm");
        time = newHM
        console.log(newHM)
    }
}
        ...
        where: {
            [models.Sequelize.Op.and]:
                [
                    ...
                    models.sequelize.where(models.sequelize.fn('date', models.sequelize.col('createdAt')), '>=', datefrom)
                ]
        },
        raw: true,
    });
    ...
    if (SunriseSunset) {
        ...
        if (feedJobs !== null) {
            ...
            if (FeederTimings[feeder_id]?.length > 0) {
                ...
                var nextSchedule = moment(mostlasttime).format("dddd, MMMM D, YYYY, H:mm:ss");
            ...
            if (Feeders.length > 0) {
                ...
                await Promise.all(
                    Feeders.map(async (feederId, i) => {
                        if (i > 0) {
                            if (FeederTimings && FeederTimings[i] && FeederTimings[i].length > 0) {
...
let HandleStopService = async (intFeederId, feederId, ToStop) => {
    ...
    if (FeedRunStatus !== null) {
        await models.FeedRunStatus.update({ status: toStop }, {
            where: {
                feeder_id: intFeederId
            }
        })
    } else {
        await models.FeedRunStatus.create({ feeder_id: intFeederId, status: toStop })
    }
...
module.exports = { ExecuteFeedingCommands, collectSchedules , getFeedDones}
