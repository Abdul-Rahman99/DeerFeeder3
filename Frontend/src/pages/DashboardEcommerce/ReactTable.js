...
const SearchTable = ({ tableData }) => {
  const columns = useMemo(
    () => [
      ...
      {
        Header: "IS Endangered",
        // accessor: "endangered",
        accessor: (cellProps) => {
          return (
            <span className="fw-semibold">
              {cellProps.endangered === 0 ? "-" : "Endangered"}
            </span>
          );
        },
        disableFilters: true,
      ...
      },
      {
        ...
        accessor: (cellProps) => {
          return (
            <span className="fw-semibold">
              {moment(cellProps.createdAt).format("YYYY-MM-DD h:mm:ss a")}
            </span>
          );
        },
        disableFilters: true,
...
// console.log("localStorage.getItem(`${feedId}-feedLevel`);" + localStorage.getItem(`${5}-feedLevel`));
const SortingTable = ({ pingDist, myData }) => {
  const [modal_backdrop, setmodal_backdrop] = useState(false);
  function tog_backdrop() {
    setmodal_backdrop(!modal_backdrop);
  }
  const sortingTable = myData;
  const columns = useMemo(
    () => [
      {
        Header: "Feeder Name",
  ...
  return (
    <React.Fragment>
      <TableContainer
        ...
        isBordered={true}
        customPageSize={5}
        className="custom-header-css table-bordered  align-middle table-nowrap"
        tableClassName="table-bordered align-middle table-nowrap mb-0"
        theadClassName="text-muted table-dark"
        SearchPlaceholder="Search Products..."
      />
    </React.Fragment>
  );
